...
function generate(myVar) {
   ...
   for(let i = 0; i < myVar; i++){
        yourIpsum = yourIpsum + lines[Math.floor(Math.random() * lines.length)] + " ";
    }
...
$('form').on('submit', function (e) {
    e.preventDefault();
    let selection = $('.shifts input[name=shift]:checked').val();
    console.log(selection);

    const final = generate(selection);
    $(".result").html(`${final}`);
    });
